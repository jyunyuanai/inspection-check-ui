...
def supports_legacy_doc_conversion() -> bool:
    ...
    雲端部署通常是 Linux，第一版雲端只支援 .docx。
    """
    return os.name == "nt"


def allowed_record_doc_exts() -> set[str]:
    return DOC_EXTS if supports_legacy_doc_conversion() else {".docx"}


def allowed_record_upload_types() -> list[str]:
    return ["doc", "docx"] if supports_legacy_doc_conversion() else ["docx"]
...
def save_record_template_files(uploaded_files: list[Any], replace_mode: bool = False) -> list[Path]:
    ...
    if errors:
        if supports_legacy_doc_conversion():
            ...
            hint = "處理方式：雲端版只支援 .docx，請先在本機用 Word 另存新檔成 .docx 後再上傳。"
        st.error(
            "有樣本匯入或轉檔失敗：\n"
            + "\n".join(errors)
            + f"\n\n{hint}"
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if supports_legacy_doc_conversion():
            ...
            st.caption("雲端版只支援 .docx。工項選單只會顯示這次匯入 Word 目錄裡的工項；重新匯入會取代目前使用者的上一批。")
